...
import { GuideMePanel } from "@/components/collabRoom/GuideMePanel";
...
function CollaborativeContent() {
  ...
  useEffect(() => {
    ...
    socket.on("activity:visited", ({ dayIndex, activityId, isVisited, visitedBy }) => {
      setTrip((prev: any) => {
        if (!prev || !prev.itinerary) return prev;
        const newItin = [...prev.itinerary];
        if (newItin[dayIndex] && newItin[dayIndex].activities) {
          const act = newItin[dayIndex].activities.find((a: any) => a._id === activityId);
          if (act) {
            act.isVisited = isVisited;
            act.visitedBy = visitedBy;
          }
        }
    ...
    socket.on("guideme:note", ({ dayIndex, activityId, note }) => {
      setTrip((prev: any) => {
        if (!prev || !prev.itinerary) return prev;
        const newItin = [...prev.itinerary];
        if (newItin[dayIndex] && newItin[dayIndex].activities) {
          const act = newItin[dayIndex].activities.find((a: any) => a._id === activityId);
          if (act) {
            if (!act.guideNotes) act.guideNotes = [];
            act.guideNotes.push(note);
          }
        }
        return { ...prev, itinerary: newItin };
      });
    ...
    return () => {
      ...
      socket.off("activity:visited");
      socket.off("guideme:note");
    };
  }, [tripId, user, socket]);

  /* ── ACTIONS ── */

  const toggleActivityVisited = async (dayIndex: number, activityId: string, currentVisited: boolean) => {
    ...
    try {
      ...
      setTrip((prev: any) => {
        ...
        const newItin = [...prev.itinerary];
        if (newItin[dayIndex] && newItin[dayIndex].activities) {
          const act = newItin[dayIndex].activities.find((a: any) => (a._id || a.id) === activityId);
          if (act) {
            act.isVisited = nextVisited;
            act.visitedBy = res.data.visitedBy;
          }
        }
        return { ...prev, itinerary: newItin };
      });
          ...
          <div className="flex-1 overflow-y-auto p-10">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="flex gap-2 p-1.5 bg-[#141416] border border-[#2a2a2e] rounded-2xl w-fit mb-10">
                {["Suggestions", "Itinerary", "Guide Me", "Polls"].map(tab => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === "Guide Me" && <Compass className="size-3.5 text-[#1D9E75]" />}
                    {tab}
                  </button>
                ))}
              </div>
              ...
              {activeTab === "Itinerary" && (() => {
                let totalSpots = 0;
                let visitedSpots = 0;
                (trip?.itinerary || []).forEach((d: any) => {
                  (d.activities || d.places || []).forEach((a: any) => {
                    totalSpots++;
                    if (a.isVisited) visitedSpots++;
                  });
                });
                const visitedPct = totalSpots > 0 ? Math.round((visitedSpots / totalSpots) * 100) : 0;

                return (
                  <div className="space-y-10">
                    <div className="p-6 rounded-3xl bg-[#141416] border border-[#2a2a2e] flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div>
                        <div className="flex items-center gap-2 text-[10px] font-black uppercase text-[#1D9E75] tracking-widest mb-1">
                          <CheckCircle2 className="size-3.5" /> Trip Progress Tracker
                        </div>
                        <h2 className="font-display font-bold text-2xl text-white">
                          {visitedSpots} of {totalSpots} Spots Visited ({visitedPct}%)
                        </h2>
                        <p className="text-xs text-[#888888] mt-1">Check off spots as your group visits them in real-time!</p>
                      </div>

                      <div className="flex items-center gap-4 min-w-[200px]">
                        <div className="flex-1 bg-[#0e0e10] h-3 rounded-full overflow-hidden border border-[#2a2a2e]">
                          <div className="bg-[#1D9E75] h-full transition-all duration-700" style={{ width: `${visitedPct}%` }} />
                        </div>
                        <button 
                          onClick={regenerateItinerary}
                          className="h-10 px-4 rounded-xl bg-[#534AB7] text-white text-xs font-bold shadow-cta hover:opacity-90 transition-all flex items-center gap-2 shrink-0"
                        >
                          <Sparkles className="size-3.5" /> AI Route
                        </button>
                      </div>
                    </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-4">
                                {(day.activities || day.places || []).map((item: any, j: number) => {
                                  const actId = item._id || item.id;
                                  return (
                                    <div key={j} className={cn(
                                      "p-5 rounded-2xl bg-[#141416] border transition-all flex items-center justify-between gap-4",
                                      item.isVisited ? "border-[#1D9E75]/50 bg-[#1D9E75]/5" : "border-[#2a2a2e]"
                                    )}>
                                      <div className="flex items-center gap-4 flex-1">
                                        {/* VISITED CHECKMARK BUTTON */}
                                        <button
                                          onClick={() => toggleActivityVisited(i, actId, !!item.isVisited)}
                                          className={cn(
                                            "size-9 rounded-xl border grid place-items-center transition-all shrink-0",
                                            item.isVisited 
                                              ? "bg-[#1D9E75] text-white border-[#1D9E75] shadow-lg shadow-[#1D9E75]/30" 
                                              : "border-[#2a2a2e] text-[#888888] hover:border-[#1D9E75] hover:text-white"
                                          )}
                                          title={item.isVisited ? "Marked as Visited" : "Mark as Visited"}
                                        >
                                          <CheckCircle2 className="size-5" />
                                        </button>

                                        <div className="text-[10px] font-black text-[#534AB7] uppercase tracking-widest bg-[#534AB7]/10 px-2.5 py-1 rounded-md shrink-0">
                                          {item.time || item.bestTime || "Morning"}
                                        </div>
                                        <div>
                                          <div className={cn("font-bold text-sm flex items-center gap-2", item.isVisited ? "line-through text-[#888888]" : "text-white")}>
                                            {item.activity || item.name || item.title || item.place}
                                            {item.isVisited && (
                                              <span className="no-underline text-[9px] font-black uppercase bg-[#1D9E75]/20 text-[#1D9E75] px-2 py-0.5 rounded-full">
                                                Visited
                                              </span>
                                            )}
                                          </div>
              ...
              {activeTab === "Guide Me" && (
                <div className="h-[650px] rounded-3xl overflow-hidden border border-[#2a2a2e]">
                  <GuideMePanel
                    tripId={tripId || ""}
                    itinerary={trip?.itinerary || []}
                    user={user}
                    socket={socket}
                    onActivityUpdate={fetchTrip}
                  />
                </div>
              )}
            </div>
